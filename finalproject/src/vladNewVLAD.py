...
def featureExtraction(X, transpose=True):
  def MFCC(signal, sr=22050):
    ...
    # mfcc_orig = librosa.feature.mfcc(y=np.array(signal), sr=sr, n_mfcc=20)
    # mfcc_h = librosa.feature.mfcc(y=y_harmonic, sr=sr, n_mfcc=20)
    # mfcc_p = librosa.feature.mfcc(y=y_percussive, sr=sr, n_mfcc=20)
...
def my_vlad(centers, X, y, transpose=True):
  ...
  if transpose:
    # Xt = np.array([ [ np.transpose(clip).flatten() for clip in song] for song in X])
  ...
  X_subtractC = min_max_scaler.fit_transform(X_subtractC)

  return X_subtractC, np.array(y_flatten)

def my_vladII(centers, X, y, transpose=True):
  def MFCC(signal, sr=22050):
    y = np.array(signal, dtype=np.float64)
  ...
  for song in Xt:
    k, dim = centers.shape
    X_subtractC = np.zeros((k, dim))
    for clip in song:
      for eachtime in clip:
        idx = getClosestDist(centers, eachtime, True)
        X_subtractC[idx] += eachtime - centers[idx]
    result.append(X_subtractC.flatten())
  result = np.array(result)
  print "result.shape", result.shape

  y_flatten = []
  for genre in y:
    tmp = [genre for i in xrange(len(result)/len(y))]
    y_flatten.extend(tmp)

  return result, np.array(y_flatten)
