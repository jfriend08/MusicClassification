...
\section{Background}
Our project focused on feature extraction using different approaches on signals.  We started with re-creating results of 2nd-order scattering \cite{mgc} , for which we have accuracy of 79\%, which is close to the results of our reference paper. %Need to add 2nd-order scattering details
...
Instead, inspired by 2nd-order scattering \cite{mgc} , we moved on to design a new method, HPSS scattering , a continous HP seperation for feature extraction.  %Need to add some details
...
From Table I. we can see various experiment results with different classifiers and different ways of featuer extraction.  Although our experiment results didn't reach the result on top of list, we have a robustness results.  % Need to add some data
